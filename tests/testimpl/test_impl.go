...
func verifyRedriveAllowPolicyExample(t *testing.T, ctx types.TestContext, sendProbeMessage bool) {
	...
	region := terraform.OutputContext(t, context.Background(), opts, "aws_region")
	...
	sourceURL := terraform.OutputContext(t, context.Background(), opts, "source_queue_url")
	sourceARN := terraform.OutputContext(t, context.Background(), opts, "source_queue_arn")
	sseMode := terraform.OutputContext(t, context.Background(), opts, "sqs_server_side_encryption")
	moduleID := terraform.OutputContext(t, context.Background(), opts, "id")
	...
	for _, url := range []string{dlqURL, sourceURL} {
		...
		case "customer_managed_kms":
			...
			kmsKeyID := terraform.OutputContext(t, context.Background(), opts, "kms_key_id")
